...
export const MainRoutes = () => {
    ...
    const PrivateForAuthorized: React.FC<IPrivateRoute> = ({ children, path }) => {
        return loadedAuth ? (
            <Route
                path={path}
                render={({ location }) =>
    ...
    return (
        <Switch>
            <PrivateForAuthorized path="/login">
                <AppRoute Component={<Login />} path={"/login"} exact Layout={LoginLayout} />
            </PrivateForAuthorized>
            <AppRoute Component={<Statistics />} path={mainRoutes.statistics.link} exact auth={{ loadedAuth, emptyAuth }} />
            <AppRoute Component={<Offereds />} path={mainRoutes.offereds.link} exact auth={{ loadedAuth, emptyAuth }} />
            <AppRoute Component={<Statistics />} path={mainRoutes.gifts.link} exact auth={{ loadedAuth, emptyAuth }} />
            <AppRoute Component={<Statistics />} path={mainRoutes.accounts.link} exact auth={{ loadedAuth, emptyAuth }} />
